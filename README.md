...
- **Heart rate:** with `hr_read` on (default) glucava reads the activity's heart rate from Strava's web session when it processes an activity, thins it to about 300 points and keeps it per activity, like the glucose readings. It shows on the activity page (average, max, min), in the activity summary email, in the activities CSV (`hr_avg`, `hr_max`, `hr_min`), and as a second curve on the chart photo (`chart_hr`). An activity without heart rate data is processed as before; a failed read is logged and never fails the run. To fill it for an activity processed earlier, reprocess it.
- **Chart lead-in:** the chart photo starts `chart_pre_minutes` (default 30) before the activity so you see where glucose came from. Only the chart uses this; the description numbers keep using `pre_minutes`.
